  ...
  return (
    <div className="bg-gray-200 rounded-2xl p-4 md:p-6">
      <div className="flex flex-wrap justify-between gap-3 mb-6">
        <button onClick={onBack} className="px-5 py-2.5 bg-gray-300 hover:bg-gray-400 rounded-lg font-semibold transition text-sm">
          ← Back to Form
        </button>
      ...
      <div className="bg-white rounded-2xl p-8 shadow-2xl">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Bank: {data?.selectedBank || "Unknown"}</p>
          <p className="text-sm text-gray-500">
            Amount: {data?.transaction?.currency || "USD"} {data?.transaction?.amount || "0"}
          </p>
          <p className="text-sm text-gray-500 mt-4">Printout untuk bank ini sedang dalam pengembangan.</p>
          <p className="text-xs text-gray-400 mt-2">Saat ini hanya HSBC yang memiliki format printout lengkap.</p>
        </div>
      </div>
    </div>
  );
