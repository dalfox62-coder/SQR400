...
const CitiForm = ({ onSubmit, initialData = {} }) => {
  const [formData, setFormData] = useState({
    institution: {
      swiftCode: initialData.institution?.swiftCode || "CITIIDJX",
      accountNumber: initialData.institution?.accountNumber || "123456789",
      accountName: initialData.institution?.accountName || "PT CITI GLOBAL ASSETS",
      bankName: initialData.institution?.bankName || "CITIBANK NA",
      address: initialData.institution?.address || "JALAN JENDERAL SUDIRMAN KAV 54-55, JAKARTA",
    },
    transaction: {
      senderReference: initialData.transaction?.senderReference || "CITI587069248914",
      transactionCode: initialData.transaction?.transactionCode || "CITIIDJX248914",
      bankOperationCode: initialData.transaction?.bankOperationCode || "CRED",
      valueDate: initialData.transaction?.valueDate || "2024-10-25",
      currency: initialData.transaction?.currency || "USD",
      amount: initialData.transaction?.amount || "1000000",
      instructedAmount: initialData.transaction?.instructedAmount || "1000000",
      remittanceInfo: initialData.transaction?.remittanceInfo || "INVESTMENT SETTLEMENT",
    ...
    beneficiary: {
      ...
      accountName: initialData.beneficiary?.accountName || "PT CITI BENEFICIARY INDO",
    ...
    sender: {
      bankName: initialData.sender?.bankName || "CITIBANK",
      senderServer: initialData.sender?.senderServer || "CITIIDJXXXX",
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-900/30 border border-slate-850 rounded-3xl p-6 md:p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-6 pb-5 border-b border-slate-855">
        <div className="w-12 h-12 bg-cyan-500/10 text-cyan-400 border border-cyan-900/30 rounded-xl flex items-center justify-center font-black text-xs tracking-wider shadow-lg">
          CITI
        </div>
        <div>
          <h2 className="text-base font-bold text-white tracking-widest uppercase">CITI NODE</h2>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="date"
              className="w-full px-3 py-2 bg-slate-950 border-2 border-slate-800 focus:border-blue-500 rounded-xl text-xs text-slate-100 outline-none focus:ring-2 focus:ring-blue-900/30"
