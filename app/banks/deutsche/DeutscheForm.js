...
const DeutscheForm = ({ onSubmit, initialData = {} }) => {
  const [formData, setFormData] = useState({
    institution: {
      ...
      accountNumber: initialData.institution?.accountNumber || "DE80100700000057105900",
      accountName: initialData.institution?.accountName || "RAVENFINANCE GMBH",
    ...
    beneficiary: {
      ...
      accountName: initialData.beneficiary?.accountName || "YAYASAN PERLINDUNGAN KONSUMEN",
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ ...formData, bankId: "deutsche", selectedBank: "Deutsche Bank", transactionDate: new Date().toLocaleString(), generatedAt: new Date().toISOString() });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      ...
      <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-200">
        <div className="w-12 h-12 bg-blue-800 rounded-lg flex items-center justify-center text-white font-bold text-sm">DB</div>
        <div>
      ...
      {/* Shortened form for demo - sama seperti BNI tapi dengan warna berbeda */}
